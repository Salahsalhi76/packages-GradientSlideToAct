...
class GradientSlideToActSalah extends StatefulWidget {
  ...
  GradientSlideToActSalah({
    ...
    required this.backgroundColor
        ...
        assert(iconSize<=height,"the size of the icon {iconSize} should be < height"),
        super(key: key);

  @override
  State<GradientSlideToActSalah> createState() => _GradientSlideToActSalahState();
}

class _GradientSlideToActSalahState extends State<GradientSlideToActSalah> with SingleTickerProviderStateMixin{
  /// default Values
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      child: ClipRRect(
          ...
          child:  _submitted?  Icon(widget.submittedIcon ?? Icons.done) :
          Stack(
            children: [
              Positioned(
                  top: 0,
              ...
              Positioned(
                  ...
                  bottom: 0,
                  child: Opacity(
                    opacity: 1- (position_percent>1? 1 : position_percent),
                    child: Center(
                      ...
                      GestureDetector(
                          onPanUpdate: (details)async{
                            ...
                            if(_position<0) {
                              _position = 0;
                              setState(() {});
                            }else if(_position>=(widget.width-widget.height-20)){
                              _position = widget.width-widget.height;
                              if(!_submitted){
                                _submitted = true;
                                setState(() {});
                                Duration _dur = const Duration(milliseconds:  200) +( widget.animationDuration);
                          ...
                          child: AnimatedPadding(
                              ...
                              child: widget.draggableWidget ?? _draggableWidget(iconSize: widget.height,gradient: widget.gradient)
                          )
                      ),
                    ],
                  )
...
class _draggableWidget extends StatelessWidget {
  final double iconSize;
  final Gradient? gradient;
  const _draggableWidget({Key? key, required this.iconSize, this.gradient}) : super(key: key);
